...
function postToApi(xmlBody) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      method: "POST",
      headers: {
        // DPO's CloudFront distribution expects text/xml, not application/xml
        "Content-Type": "text/xml; charset=utf-8",
        "Content-Length": postData.length,
        "Accept": "text/xml, application/xml",
        // Mimic a browser-like User-Agent to pass CloudFront WAF rules
        "User-Agent":
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", async () => {
        // Log HTTP status for debugging — remove in production
        console.log(`[DPO] HTTP ${res.statusCode} | Content-Type: ${res.headers["content-type"]}`);

        if (res.statusCode !== 200) {
          return reject(
            new Error(`DPO API returned HTTP ${res.statusCode}.\nBody: ${raw.slice(0, 300)}`)
          );
        }
